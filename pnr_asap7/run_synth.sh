#!/bin/bash
# run_synth.sh - Yosys synthesis + post-synth STA for rv32imc_top on ASAP7.
# Usage: ./run_synth.sh [CLOCK_PERIOD_PS]   (default 667 = 1.50 GHz)
set -euo pipefail
...
echo "=== asap7 synthesis: rv32imc_top @ ${PERIOD}ps (target $(python3 -c "print(f'{1000/$PERIOD:.2f}')") GHz) ==="
make -C "$FLOW_DIR" \
  DESIGN_CONFIG="$DESIGN_DIR/config.mk" \
  CLOCK_PERIOD="$PERIOD" \
  synth 2>&1 | tee "$RV32IMC_TOP/pnr_asap7/reports/synth_${PERIOD}ns.log"
